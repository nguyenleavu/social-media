...
const getBitrate = (filePath) => {
    return new Promise((resolve, reject) => {
        (0, child_process_1.exec)(`ffprobe -v error -select_streams v:0 -show_entries stream=bit_rate -of default=nw=1:nk=1 ${filePath}`, (err, stdout, stderr) => {
            if (err) {
                return reject(err);
            }
...
const getResolution = (filePath) => {
    return new Promise((resolve, reject) => {
        (0, child_process_1.exec)(`ffprobe -v error -select_streams v:0 -show_entries stream=width,height -of csv=s=x:p=0 ${filePath}`, (err, stdout, stderr) => {
            if (err) {
                return reject(err);
            }
